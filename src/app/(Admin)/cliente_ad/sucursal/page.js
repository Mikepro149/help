...
export default function RegistrarSucursal() {
  ...
  const step = 2; // Sucursal

  const [checked, setChecked] = useState(false); // Estado para checkbox
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Contacto',
      cell: ({ row }) => <div className="lowercase">{row.getValue('contacto')}</div>,
    },
    {
      accessorKey: 'telefono',
      header: 'Teléfono',
      cell: ({ row }) => <div className="text-right font-medium">{row.getValue('telefono')}</div>,
    },
    {
      accessorKey: 'correo',
      header: 'Correo',
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <input
          ...
          onChange={(e) => {
            if (e.target.checked) {
              window.location.href = '/cliente_ad/area';
            }
          }}
        />
      ),
  ...
  // Manejar cambio de checkbox y navegación
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <button type="button" onClick={() => window.history.back()} style={navBtnStyle}>
          Cancelar
        </button>
        <button
          type="button"
          onClick={() => (window.location.href = '/cliente_ad/acceso')}
          style={navBtnStyle}
        >
          Siguiente
        </button>
